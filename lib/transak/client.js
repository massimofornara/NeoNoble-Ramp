import { getTransakEnvironment, getTransakUrls, requireServerTransakConfig } from './config.js';

let accessTokenCache = null;

async function transakFetch(url, init = {}) {
...
export async function getTransakAccessToken() {
  ...
  const response = await transakFetch(`${apiBaseUrl}/partners/api/v2/refresh-token`, {
    method: 'POST',
    headers: {
...
export function toOfficialWidgetParams(input, origin) {
  const config = requireServerTransakConfig();
...
export async function createWidgetSession(input, origin) {
  ...
  const response = await transakFetch(`${apiGatewayUrl}/api/v2/auth/session`, {
    method: 'POST',
    headers: {
...
export async function getOrderById(orderId) {
  const { apiBaseUrl } = getTransakUrls();
  const accessToken = await getTransakAccessToken();
  return transakFetch(`${apiBaseUrl}/partners/api/v2/order/${encodeURIComponent(orderId)}`, {
    method: 'GET',
    headers: {
      'access-token': accessToken,
    },
...
export async function getOrders(searchParams) {
  const { apiBaseUrl } = getTransakUrls();
  const accessToken = await getTransakAccessToken();
  return transakFetch(`${apiBaseUrl}/partners/api/v2/orders?${searchParams.toString()}`, {
    method: 'GET',
    headers: {
      'access-token': accessToken,
    },
